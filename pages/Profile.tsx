...
export const Profile: React.FC = () => {
  ...
  // If no userId param is present, view my own profile
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [targetUserId, currentUser?.id]); // Depend on targetUserId
  ...
  if (!profileUser) {
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-4">
            ...
            <h2 className="text-lg font-bold text-slate-700 mb-2">사용자를 찾을 수 없습니다</h2>
            <button onClick={() => navigate('/')} className="text-brand-600 font-bold hover:underline">홈으로 돌아가기</button>
        </div>
    );
  }
  ...
  const successPlans = plans.filter(p => p.status === PlanStatus.COMPLETED_SUCCESS).length;
  ...
  // Calculate Trust Score (Average "Can Do" Percentage)
  let trustScore = 0;
  let totalVotedPlans = 0;
  
  plans.forEach(p => {
      const totalVotes = p.votes.canDoIt + p.votes.cannotDoIt;
      if (totalVotes > 0) {
          trustScore += (p.votes.canDoIt / totalVotes);
          totalVotedPlans++;
      }
  });
  
  const finalTrustScore = totalVotedPlans > 0 ? Math.round((trustScore / totalVotedPlans) * 100) : 0;
  ...
  // Helper to calculate simple progress for display
        ...
        <div className="max-w-4xl mx-auto px-4 relative">
            <div className="absolute -top-12 p-1 bg-white rounded-full">
                <img 
                    ...
                    alt="Profile" 
                    className="w-24 h-24 rounded-full border border-slate-100 shadow-md bg-slate-50"
                />
            </div>
            <div className="pt-16">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <div className="mb-4 md:mb-0">
                        ...
                        <p className="mt-3 text-slate-700 max-w-lg leading-relaxed text-sm whitespace-pre-wrap">
                            {profileUser.bio}
                        </p>
                    </div>
                    ...
                    {isMe ? (
                        <div className="flex space-x-2 w-full md:w-auto mt-2 md:mt-0">
                            ...
                            <button 
                                ...
                                className="flex-1 md:flex-none border border-slate-200 bg-white text-red-500 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-50 hover:border-red-200 transition-colors flex items-center justify-center"
                            >
                        ...
                        <button 
                            ...
                            className={`w-full md:w-auto px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center shadow-sm mt-2 md:mt-0 ${
                                isFollowing 
                        ...
                        >
                            {followLoading ? <Loader2 className="animate-spin" size={16} /> : (
                                isFollowing ? (
                                    <><UserCheck size={16} className="mr-2"/> 팔로잉</>
                                ) : (
                                    <><UserPlus size={16} className="mr-2"/> 팔로우</>
                                )
                            )}
                        </button>
                    )}
                        ...
                        return (
                            <div key={plan.id} onClick={() => navigate(`/plan/${plan.id}`)} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow cursor-pointer group">
                                ...
                                <div className="flex justify-between text-xs text-slate-500 font-medium">
                                    ...
                                    <span>마일스톤 {completedMilestones}/{plan.milestones.length} 달성</span>
                                </div>
                            </div>
                        );
                    })}
                     ...
                     {pastPlans.map(plan => (
                         <div key={plan.id} onClick={() => navigate(`/plan/${plan.id}`)} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 opacity-80 hover:opacity-100 transition-opacity cursor-pointer">
                             <div className="flex justify-between items-center">
                                 ...
                                 {plan.status === PlanStatus.COMPLETED_SUCCESS ? (
                                     <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded font-bold">성공</span>
                                 ) : (
                                     <span className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded font-bold">실패</span>
                                 )}
                             </div>
